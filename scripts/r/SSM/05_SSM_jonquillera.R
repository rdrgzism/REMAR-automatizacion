...
# Reviewed and documented: Ismael Rodríguez & Jose María Disder
# Date: 2025-05-08
...
# --- Configuration
metier <- "jonquillera" #cambia por ser el output del anterior
period <- 2 * 60  # seconds between interpolated points (adjust per metier)
...
df <- df %>%
  mutate(group = cumsum(lag(hat, default = first(hat)) != hat)) %>%
  group_by(group) %>%
  mutate(group_size = n()) %>%
  ungroup()
...
df <- df %>%
  mutate(hat_grouped = case_when(
    hat == 3 & group_size > 2 ~ 3L,
    TRUE ~ 2L
  ))

# Si hat es igual a "3" y group_size es mayor que 3, entonces hat_grouped será "3".
# En cualquier otro caso (TRUE es el caso por defecto), hat_grouped será "2".

# Filtrar los grupos de hat == 3 que tengan más de 4 elementos
# Y numerar secuencialmente cada grupo completo
df <- df %>%
  group_by(group) %>%
  mutate(group_numbered = ifelse(hat_grouped == 3 & group_size > 2, group, NA)) %>%
  ungroup() %>%
  mutate(group_numbered = dense_rank(group_numbered))

df_completo <- rbind(df)

df_dissolved <- df %>%
  group_by(group_numbered) %>%
  summarise(
    hat = first(hat),
    x = first(X),
    y = first(Y),
    ...
    hat_grouped = first(hat_grouped),
    # Tomar el primer valor de 'hat'
    count = n(),                           # Contar el número de filas en el grupo
    total_group_size = sum(group_size)      # Sumar el tamaño de los grupos en ese 'group_numbered'
  )

df_dissolved <- df_dissolved %>%
  filter(!is.na(group_numbered))

df_dissolved <- df_dissolved %>%
  group_by(journey) %>%
  arrange(time, .by_group = TRUE) %>%     # Ordena por fecha/hora o el criterio que uses
  mutate(
    Lance = row_number(),                        # Numeración de lance por journey
    Max_lance = max(Lance)                       # Máximo lance por journey
  ) %>%
  ungroup() %>%
  mutate(Lance = factor(Lance))                   # Convertir Lance a factor (opcional)
...
st_crs(points) <- st_crs(p_units)
intersected <- st_intersection(points, p_units[, c("ID_GRID")])

# Save shapefile
year_str <- year(min(input$time, na.rm = TRUE))
st_write(sf_dissolved, file.path(results_dir, paste0(metier, "_dissolved_",
                                                     year_str, ".shp")),
         delete_layer = TRUE)
st_write(intersected, file.path(results_dir, paste0(metier, "_intersected_",
                                                    year_str, ".shp")),
         delete_layer = TRUE)

save(df_completo, df_dissolved, intersected,
