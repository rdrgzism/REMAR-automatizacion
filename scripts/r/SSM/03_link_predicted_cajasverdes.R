...
# --- Load data
if (file.exists(file.path(results_dir, "predicted.RData"))) {
  load(file.path(results_dir, "predicted.RData"))         # OUT and journey_list
} else{
...
process_gps_data <- function(end_date = NULL, n_days = 1, tracks_dir, 
  ...
  if (!is.null(end_date)) {
    ...
    gps_date <- seq(end_date - n_days + 1, end_date, by = "day") - 1
  } else {
  ...
  selected_files <- track_files[dates_parsed %in% gps_date]
  
  if (length(selected_files) == 0) {
    warning(paste("No GPS data found for date(s)", paste(gps_date, collapse = ", "), "\n"))
    return(NULL)
  }
  
  # Leer todos los archivos seleccionados con columnas explícitas
